...
export const AppLayout = () => {
    ...
    const currentUser = getSessionValue<CurrentUserInfo>('currentUser');
                    ...
                    <nav className="flex flex-col gap-6">
                        {groupedRoutes.map(({ group, routes }) => (
                            <div key={group} className="flex flex-col gap-2">
                                {!collapsed ? (
                                    <p className="px-3 text-[11px] font-semibold uppercase tracking-[0.24em] text-muted-foreground/80">{group}</p>
                                ) : null}
                                            ...
                                            <NavLink
                                                ...
                                                className={({ isActive }) =>
                                                    cn(
                                                        ...
                                                        isActive
                                                            ? 'bg-foreground text-background shadow-[0_14px_40px_rgba(15,23,42,0.18)]'
                        ...
                        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div className="flex items-center gap-3 rounded-full border border-border/70 bg-card/80 px-2 py-1.5">
                                    <Avatar>
                                        <AvatarImage src={currentUser?.avatar || logoUrl} alt={currentUser?.name ?? 'admin'} />
                                        <AvatarFallback>{(currentUser?.name ?? 'PX').slice(0, 2)}</AvatarFallback>
                                    </Avatar>
                                    <div className="hidden pr-3 md:block">
                                        <p className="text-sm font-semibold">{currentUser?.name ?? 'Admin'}</p>
                                        <p className="text-xs text-muted-foreground">{currentUser?.roleList?.[0]?.roleName ?? 'super administrator'}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
