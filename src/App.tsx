...
function PublicOnlyRoute({ children }: { children: ReactNode }) {
    const location = useLocation();
    const token = getToken();
    const redirectTarget = getSafeRedirectTarget(new URLSearchParams(location.search).get('redirect')) ?? '/home';
        ...
        <Routes>
            ...
            >
                ...
                {appRoutes
                    .filter(
                        (route) =>
                            ![
                                ...
                                '/admin/log/index',
                                '/admin/file/index',
                                '/gen/datasource/index',
                                '/gen/field-type/index',
                                '/gen/group/index',
                                '/gen/table/index',
                                '/gen/template/index',
                                '/gen/design/index',
                    ...
                    .map((route) => (
                        <Route key={route.path} path={route.path.slice(1)} element={<ResourcePage meta={findRouteMetaByPath(route.path)!} />} />
                    ))}
            </Route>
