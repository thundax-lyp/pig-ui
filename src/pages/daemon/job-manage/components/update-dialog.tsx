			...
			<DialogContent className="max-w-3xl">
				...
				<div className="grid gap-4 md:grid-cols-2">
					...
					<FormField label="任务类型" error={errors.jobType}>
						<select className="h-12 rounded-2xl border border-border/70 bg-background px-4 text-sm" value={form.jobType} onChange={(event) => onChange((prev) => ({ ...prev, jobType: event.target.value }))}>
					...
					{isPathJob ? (
						<FormField label="执行路径" error={errors.executePath}>
							<Input value={form.executePath} onChange={(event) => onChange((prev) => ({ ...prev, executePath: event.target.value }))} />
						</FormField>
					) : null}
					{isBeanJob ? (
						<>
							...
							<FormField label="方法名" error={errors.methodName}>
								<Input value={form.methodName} onChange={(event) => onChange((prev) => ({ ...prev, methodName: event.target.value }))} />
							</FormField>
						</>
					) : null}
					<FormField label="方法参数">
						<Input value={form.methodParamsValue} onChange={(event) => onChange((prev) => ({ ...prev, methodParamsValue: event.target.value }))} />
					</FormField>
					<FormField label="Cron" error={errors.cronExpression}>
						<Input value={form.cronExpression} onChange={(event) => onChange((prev) => ({ ...prev, cronExpression: event.target.value }))} placeholder="0 0/5 * * * ?" />
					</FormField>
					<FormField label="失火策略" error={errors.misfirePolicy}>
						<select className="h-12 rounded-2xl border border-border/70 bg-background px-4 text-sm" value={form.misfirePolicy} onChange={(event) => onChange((prev) => ({ ...prev, misfirePolicy: event.target.value }))}>
							<option value="">请选择</option>
