    ...
    return (
        <div className="flex flex-col gap-6">
                    ...
                    <div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_220px_220px_auto_auto]">
                        <div className="relative">
                            ...
                            <Input
                                ...
                                onChange={(event) => setFilters((prev) => ({ ...prev, jobName: event.target.value }))}
                                placeholder="按任务名称搜索"
                            />
                        </div>
                        <select
                            ...
                            onChange={(event) => setFilters((prev) => ({ ...prev, jobStatus: event.target.value }))}
                        >
                        ...
                        <select
                            className="h-12 rounded-2xl border border-border/70 bg-background px-4 text-sm"
                            value={filters.jobExecuteStatus}
                            onChange={(event) => setFilters((prev) => ({ ...prev, jobExecuteStatus: event.target.value }))}
                        >
                                ...
                                <thead className="bg-secondary/70 text-left text-muted-foreground">
                                    <tr>
                                        <th className="px-4 py-3">
                                            <input
                                                type="checkbox"
                                                checked={jobs.length > 0 && selectedRows.length === jobs.length}
                                                onChange={(event) => setSelectedRows(event.target.checked ? jobs : [])}
                                            />
                                        </th>
                                        ...
                                        jobs.map((row) => (
                                            <tr key={row.jobId} className="border-t border-border/60 bg-background/70">
                                                <td className="px-4 py-4 align-top">
                                                    <input
                                                        type="checkbox"
                                                        checked={selectedRows.some((item) => item.jobId === row.jobId)}
                                                        onChange={(event) =>
                                                            setSelectedRows((prev) =>
                                                                event.target.checked
                                                                    ? [...prev, row]
                                                                    : prev.filter((item) => item.jobId !== row.jobId)
                                                            )
                                                        }
                                                    />
                                                </td>
                                                ...
                                                <td className="px-4 py-4 align-top">
                                                    <Badge variant="outline">
                                                        {jobExecuteStatusMap.get(row.jobExecuteStatus) ?? row.jobExecuteStatus}
                                                    </Badge>
                                                </td>
                                                ...
                                                <td className="px-4 py-4 align-top text-muted-foreground">
                                                    {misfirePolicyMap.get(row.misfirePolicy) ?? row.misfirePolicy}
                                                </td>
                                                <td className="px-4 py-4 align-top">
            ...
            <JobFormDialog
                ...
                misfirePolicyOptions={misfirePolicyOptions}
                ...
                <DialogContent className="max-w-6xl">
                    ...
                    <div className="flex items-center justify-between gap-3">
                        ...
                        <Button
                            ...
                            onClick={() => void handleDeleteLogs(selectedLogIds)}
                        >
                            <Trash2 data-icon="inline-start" />
                            删除日志
                        </Button>
                                ...
                                <thead className="bg-secondary/70 text-left text-muted-foreground">
                                    <tr>
                                        <th className="px-4 py-3">
                                            <input
                                                type="checkbox"
                                                checked={jobLogs.length > 0 && selectedLogIds.length === jobLogs.length}
                                                onChange={(event) =>
                                                    setSelectedLogIds(event.target.checked ? jobLogs.map((item) => item.jobLogId) : [])
                                                }
                                            />
                                        </th>
                                        <th className="px-4 py-3">任务名称</th>
                                        ...
                                        jobLogs.map((row) => (
                                            <tr key={row.jobLogId} className="border-t border-border/60 bg-background/70">
                                                <td className="px-4 py-4 align-top">
                                                    <input
                                                        type="checkbox"
                                                        checked={selectedLogIds.includes(row.jobLogId)}
                                                        onChange={(event) =>
                                                            setSelectedLogIds((prev) =>
                                                                event.target.checked
                                                                    ? [...prev, row.jobLogId]
                                                                    : prev.filter((id) => id !== row.jobLogId)
                                                            )
                                                        }
                                                    />
                                                </td>
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-3">
                        ...
                        <div className="flex gap-3">
                            <Button
                                variant="outline"
                                onClick={() => activeJob && loadJobLogs(activeJob.jobId, jobLogPage - 1)}
                                disabled={jobLogPage <= 1 || !activeJob || submitting}
                            >
                                上一页
                            </Button>
                            <Button
                                variant="outline"
                                onClick={() => activeJob && loadJobLogs(activeJob.jobId, jobLogPage + 1)}
                                disabled={jobLogPage >= jobLogPages || !activeJob || submitting}
                            >
                                下一页
                            </Button>
                        </div>
