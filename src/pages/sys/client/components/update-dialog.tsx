            ...
            <DialogContent className="max-w-3xl">
                ...
                <div className="grid gap-4 md:grid-cols-2">
                    ...
                    <FormField label="accessTokenValidity" error={errors.accessTokenValidity}>
                        <Input
                            type="number"
                            value={String(form.accessTokenValidity)}
                            onChange={(event) => onChange((prev) => ({ ...prev, accessTokenValidity: Number(event.target.value || 0) }))}
                        />
                    </FormField>
                    <FormField label="refreshTokenValidity" error={errors.refreshTokenValidity}>
                        <Input
                            type="number"
                            value={String(form.refreshTokenValidity)}
                            onChange={(event) => onChange((prev) => ({ ...prev, refreshTokenValidity: Number(event.target.value || 0) }))}
                        />
                    </FormField>
                                ...
                                return (
                                    <button
                                        ...
                                        onClick={() =>
                                            onChange((prev) => ({
                                                ...prev,
                                                authorizedGrantTypes: active
                                                    ? prev.authorizedGrantTypes.filter((value) => value !== item.value)
                                                    : [...prev.authorizedGrantTypes, item.value],
                                            }))
                                        }
                                    >
                                        {item.label}
                    ...
                    <div className="grid gap-4 md:grid-cols-2">
                        ...
                        <FormField label="authorities">
                            <Input
                                ...
                                onChange={(event) => onChange((prev) => ({ ...prev, authorities: event.target.value }))}
                            />
                        </FormField>
                        <FormField label="回调地址" error={errors.webServerRedirectUri}>
                            <Input
                                value={form.webServerRedirectUri}
                                onChange={(event) => onChange((prev) => ({ ...prev, webServerRedirectUri: event.target.value }))}
                            />
                        </FormField>
                    </div>
                ) : null}
