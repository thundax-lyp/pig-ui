				...
				<CardContent className="flex flex-col gap-4">
					...
					<div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_auto_auto]">
						<div className="relative">
							...
							<Input className="pl-10" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="按原文件名搜索" />
						</div>
								...
								<thead className="bg-secondary/70 text-left text-muted-foreground">
									<tr>
										<th className="px-4 py-3">
											<input type="checkbox" checked={files.length > 0 && selectedIds.length === files.length} onChange={(event) => setSelectedIds(event.target.checked ? files.map((item) => item.id) : [])} />
										</th>
				...
				<DialogContent className="max-w-2xl">
						...
						<div className="flex flex-col gap-2">
							{pendingFiles.length === 0 ? (
								...
								pendingFiles.map((file) => (
									<div key={`${file.name}-${file.size}`} className="rounded-2xl border border-border/70 bg-background/70 px-4 py-3 text-sm">
					...
					<DialogFooter>
						...
						<Button onClick={() => void handleUpload()} disabled={!pendingFiles.length || submitting}>
							{submitting ? <LoaderCircle className="h-4 w-4 animate-spin" data-icon="inline-start" /> : <Upload data-icon="inline-start" />}
							开始上传
						</Button>
					</DialogFooter>
				</DialogContent>
