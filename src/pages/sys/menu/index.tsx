				...
				<CardContent className="flex flex-col gap-4">
					...
					<div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_auto_auto]">
						<div className="relative">
							...
							<Input className="pl-10" value={draftQuery} onChange={(event) => setDraftQuery(event.target.value)} placeholder="按菜单名称搜索" />
						</div>
					...
					{form.menuType === '0' ? (
						<>
							...
							<FormField label="图标" error={errors.icon}>
								<Input value={form.icon} onChange={(event) => onChange({ ...form, icon: event.target.value })} placeholder="例如 icon-shouye" />
							</FormField>
						</>
					) : (
						<FormField label="权限标识" error={errors.permission}>
							<Input value={form.permission} onChange={(event) => onChange({ ...form, permission: event.target.value })} placeholder="例如 sys_menu_add" />
						</FormField>
					)}
					<FormField label="排序">
						<Input
