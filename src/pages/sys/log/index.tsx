			...
			<Card>
				...
				<CardContent className="flex flex-col gap-4">
					...
					<div className="grid gap-3 md:grid-cols-[220px_minmax(0,1fr)_minmax(0,1fr)_auto_auto]">
						...
						<Input type="datetime-local" value={filters.start} onChange={(event) => setFilters((prev) => ({ ...prev, start: event.target.value }))} />
						<Input type="datetime-local" value={filters.end} onChange={(event) => setFilters((prev) => ({ ...prev, end: event.target.value }))} />
								...
								<thead className="bg-secondary/70 text-left text-muted-foreground">
									<tr>
										<th className="px-4 py-3">
											<input type="checkbox" checked={logs.length > 0 && selectedIds.length === logs.length} onChange={(event) => setSelectedIds(event.target.checked ? logs.map((item) => item.id) : [])} />
										</th>
