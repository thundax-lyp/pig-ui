				...
				<CardContent className="flex h-[calc(100%-5rem)] flex-col gap-4">
					<div className="flex flex-col gap-3">
						<div className="relative">
							...
							<Input className="pl-10" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="搜索字典类型或名称" />
						</div>
								...
								{dictTypes.map((item) => (
									...
									>
										<div className="min-w-0">
											...
											<p className={cn('mt-1 truncate text-xs', selectedType?.id === item.id ? 'text-background/70' : 'text-muted-foreground')}>{item.dictType}</p>
										</div>
			...
			<Card className="min-h-[720px]">
				...
				<CardContent className="flex flex-col gap-4">
					...
					<div className="flex flex-wrap items-center justify-between gap-3">
						...
						<div className="flex gap-3">
							<Button variant="outline" onClick={() => void loadDictItems(selectedType, page - 1)} disabled={page <= 1 || loadingItems || !selectedType}>
								上一页
							</Button>
							<Button variant="outline" onClick={() => void loadDictItems(selectedType, page + 1)} disabled={page >= totalPages || loadingItems || !selectedType}>
								下一页
							</Button>
						</div>
					</div>
