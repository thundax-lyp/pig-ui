...
export const ParamFormDialog = ({ open, form, errors, submitting, dictTypeOptions, statusOptions, paramTypeOptions, onOpenChange, onChange, onSubmit }: Props) => {
	return (
			...
			<DialogContent className="max-w-3xl">
				...
				<div className="grid gap-4 md:grid-cols-2">
					<FormField label="系统标识" error={errors.systemFlag}>
						<div className="flex flex-wrap gap-2">
							{dictTypeOptions.map((option) => (
								<ToggleChip key={option.value} active={form.systemFlag === option.value} onClick={() => onChange((prev) => ({ ...prev, systemFlag: option.value }))}>
									{option.label}
								</ToggleChip>
							))}
						</div>
					...
					<FormField label="参数类型" error={errors.publicType}>
						<div className="flex flex-wrap gap-2">
							{paramTypeOptions.map((option) => (
								<ToggleChip key={option.value} active={form.publicType === option.value} onClick={() => onChange((prev) => ({ ...prev, publicType: option.value }))}>
									{option.label}
								</ToggleChip>
							))}
						</div>
					...
					<FormField label="校验编码">
						<Input value={form.validateCode || ''} onChange={(event) => onChange((prev) => ({ ...prev, validateCode: event.target.value }))} />
					</FormField>
					...
					<FormField label="参数键" error={errors.publicKey}>
						<Input value={form.publicKey} onChange={(event) => onChange((prev) => ({ ...prev, publicKey: event.target.value.toUpperCase() }))} />
					</FormField>
					...
					<FormField label="状态" error={errors.status}>
						<div className="flex flex-wrap gap-2">
							{statusOptions.map((option) => (
								<ToggleChip key={option.value} active={form.status === option.value} onClick={() => onChange((prev) => ({ ...prev, status: option.value }))}>
									{option.label}
								</ToggleChip>
							))}
						</div>
