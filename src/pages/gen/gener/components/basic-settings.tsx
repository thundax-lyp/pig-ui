	...
	return (
		<div className="grid gap-4 md:grid-cols-2">
			...
			<FormField label="代码风格">
				<select className="h-12 rounded-2xl border border-border/70 bg-background px-4 text-sm" value={form.style} onChange={(event) => onChange((prev) => ({ ...prev, style: event.target.value }))}>
					...
					{[
						...
						<button
							...
							className={cn(
								...
								form.formLayout === item.value ? 'border-primary bg-primary text-primary-foreground' : 'border-border/70 bg-background text-muted-foreground hover:text-foreground'
							)}
				...
				<div className="flex flex-wrap gap-2">
					{[
						...
						<button
							...
							className={cn(
								'rounded-full border px-4 py-2 text-sm transition',
								form.generatorType === item.value ? 'border-primary bg-primary text-primary-foreground' : 'border-border/70 bg-background text-muted-foreground hover:text-foreground'
							)}
