...
import {
    fetchDictList,
    fetchGeneratorPreview,
    fetchTableDetail,
    generateCode,
    updateTableBase,
    updateTableFields,
    downloadCodeZip,
} from '@/pages/gen/table/service';
...
export const GenGenerPage = () => {
    ...
    const loadInitialData = async () => {
        try {
            ...
            setAttrTypeOptions(
                ['String', ...Array.from(new Set((fieldTypeResponse.data ?? []).map((item) => item.attrType))), 'Object'].filter(
    ...
    const validateBaseForm = useMemo(
        () => () => {
            ...
            if (baseForm.generatorType === '1' && (!baseForm.backendPath.trim() || !baseForm.frontendPath.trim()))
                return '自定义路径生成时，前后端路径不能为空。';
            return '';
        },
        [baseForm]
    );
                        ...
                        {step === 0 ? (
                            ...
                            <>
                                ...
                                <Button
                                    ...
                                    onClick={() =>
                                        window.open(
                                            `/gen/design/index?tableName=${encodeURIComponent(tableName)}&dsName=${encodeURIComponent(dsName)}`,
                                            '_self'
                                        )
                                    }
                                >
