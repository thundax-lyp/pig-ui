...
import { createFieldType, deleteFieldTypes, exportFieldTypes, fetchFieldTypeDetail, fetchFieldTypeDetailByColumnType, fetchFieldTypePage, updateFieldType } from './service';
				...
				<CardContent className="flex flex-col gap-4">
					...
					<div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_auto_auto]">
						<div className="relative">
							...
							<Input className="pl-10" placeholder="按字段类型搜索" value={search} onChange={(event) => setSearch(event.target.value)} />
						</div>
							...
							<table className="min-w-full bg-card/70 text-sm">
								...
								<tbody>
									{loading ? (
										...
										<tr><td colSpan={6} className="px-4 py-16 text-center text-muted-foreground">没有符合条件的字段类型。</td></tr>
									) : rows.map((row) => (
