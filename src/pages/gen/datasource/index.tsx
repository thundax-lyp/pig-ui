...
import { Database, Download, LoaderCircle, Pencil, Plus, Search, Trash2 } from 'lucide-react';
...
export const GenDatasourcePage = () => {
	...
	return (
		<div className="flex flex-col gap-6">
				...
				<CardContent className="flex flex-col gap-4">
					...
					<div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_auto_auto]">
						<div className="relative">
							...
							<Input className="pl-10" placeholder="按数据源名搜索" value={search} onChange={(event) => setSearch(event.target.value)} />
						</div>
								...
								<thead className="bg-secondary/70 text-left text-muted-foreground">
									<tr>
										<th className="px-4 py-3">
											<input type="checkbox" checked={rows.length > 0 && selectedIds.length === rows.length} onChange={(event) => setSelectedIds(event.target.checked ? rows.map((item) => item.id) : [])} />
										</th>
										...
										rows.map((row) => (
											<tr key={row.id} className="border-t border-border/60 bg-background/70">
												<td className="px-4 py-4 align-top">
													<input type="checkbox" checked={selectedIds.includes(row.id)} onChange={(event) => setSelectedIds((prev) => (event.target.checked ? [...prev, row.id] : prev.filter((id) => id !== row.id)))} />
												</td>
			...
			<UpdateDialog open={dialogOpen} form={form} errors={errors} submitting={submitting} onOpenChange={setDialogOpen} onChange={(updater) => setForm((prev) => updater(prev))} onSubmit={() => void handleSave()} />
		</div>
	);
};
